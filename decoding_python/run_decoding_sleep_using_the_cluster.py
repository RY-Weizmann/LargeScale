...
def create_job_submission_str(exp_ID, opt_params, num_epochs, epochs_to_decode, folder):
    ...
    output_dir = Path('..') / 'jobs_output' / timestr #/ 'opt_{}'.format(opt_params)
    output_dir.mkdir(parents=True, exist_ok=True)
    out_file =  output_dir / 'opt_{}.txt'.format(opt_params)
    ...
    bsub_opts = (
        ...
        # f' -R "affinity[thread*{NUMBA_NUM_THREADS}]"'
        f' -We {wall_time}'
        # Raz 3-9-25: does not work f' -outdir "../jobs_output/%J/"'
        # Raz 3-9-25: does not work f' -o "../jobs_output/%J/%J_%I.txt"'
        f' -outdir {output_dir}'
        f' -o {out_file}'
